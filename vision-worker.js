...
/* Dual-environment: inside a Web Worker this file importScripts the engine
   and speaks postMessage; under Node (the AWS Lambda eval harness) the
   caller sets global.cv to the resolved opencv.js module first and calls
   the exported analyze() directly — same maths, same numbers, no browser. */
const IS_WORKER = typeof importScripts === "function";
if (IS_WORKER) importScripts("opencv.js");

let ready = false, failed = false;
const pending = []; // {id, imageData} records — NOT closures, so buffers stay collectable

if (IS_WORKER) (function waitForCv() {
...
function detectCellsByContour(gray, inverted, bias, kernelDiv = 3, openSize = 5, gates = {}) {
  ...
  try {
    ...
    if (typeof self !== "undefined" && self.__dumpBins && !inverted) {
